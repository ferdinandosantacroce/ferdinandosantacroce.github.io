---
title: "Lavori"
description: "Produzioni e opere letterarie di Ferdinando Santacroce."
slug: "lavori"
date: 2022-03-06 00:00:00+0000
image: cover-git-apogeo.jpg
toc: true
menu:
    main:
        weight: 3
        params: 
            icon: brain
---

Mi piace leggere e mi piace anche scrivere. Ecco alcune delle ultime cose a cui ho lavorato.

## Blog di Semaphore
Scrivo di testing sul blog di [Semaphore](https://semaphoreci.com/): [qui puoi trovare i mio articoli](https://semaphoreci.com/author/ferdinando-santacroce).

## Git Essentials
Qualche tempo fa ho scritto un libro per [Packt Publishing](https://www.packtpub.com/) su *Git*.

[Git Essentials](https://www.packtpub.com/application-development/git-essentials).

![Git Essentials](git-essentials-1st-edition-small.png "Git Essentials 1st Edition")

Il libro è stato incluso nella [Git Survey 2016](https://git.wiki.kernel.org/index.php/GitSurvey2016), ottenendo il [3° posto](https://survs.com/report/nz2odu1spl) (a anni luce di distanza da [Pro Git](https://git-scm.com/book/en/v2), ovviamente :sweat_smile:).

---

Mi è piaciuto molto scriverlo e ho imparato tanto. Così ho deciso di scriverlo di nuovo :smiley:

[Git Essentials, 2nd Edition](https://www.packtpub.com/application-development/git-essentials-second-edition).

![Git Essentials, 2nd Edition](git-essentials-2nd-edition-small.png "Git Essentials 2nd Edition")

Anche questa volta il libro è stato apprezzato.
<a class="ba-award" href="https://bookauthority.org/books/best-git-books?t=10kfmh&s=award&book=1787120724" target="_blank" style="margin:20px; outline:0"><img src="https://award.bookauthority.org/best-git-books.png?b=1787120724&c=1&v=6&w=200" style="width:200px; height:183px; border:0" alt="BookAuthority Best Git Books of All Time"/></a>

---

Il libro è stato tradotto in italiano da Apogeo:

[Git - Guida per imparare a gestire, distribuire e versionare codice](https://www.apogeonline.com/libri/git-ferdinando-santacroce/).

![Git - Guida per imparare a gestire, distribuire e versionare codice](cover-git-apogeo.jpg "Git - Guida per imparare a gestire, distribuire e versionare codice")

---

Seguimi su [Goodreads](https://www.goodreads.com/author/show/13649672.Ferdinando_Santacroce) per saperne di più sui libri che leggo e che scrivo.

## Mokabyte
Sono un autore tecnico per [Mokabyte](https://www.mokabyte.it), una webzine italiana con vent'anni di storia. Qui trovi la mia serie su [Git, lo stupido tracciatore di contenuti](https://www.mokabyte.it/author/ferdinando-santacroce/).